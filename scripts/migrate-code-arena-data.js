      ...
      const learningActivity = await prisma.learningActivity.create({
        data: {
          ...
          content: JSON.stringify({
            ...
            testCases: safeJsonParse(arena.testCases, []),
            hints: safeJsonParse(arena.hints, []),
            examples: safeJsonParse(arena.examples, []),
            sections: safeJsonParse(arena.sections, []),
            learningObjectives: safeJsonParse(arena.learningObjectives, []),
            resources: safeJsonParse(arena.resources, []),
          }),
          settings: JSON.stringify({
            duration: arena.duration,
            hasCodeExercise: arena.hasCodeExercise,
            prerequisites: safeJsonParse(arena.prerequisites, []),
          }),
...
function safeJsonParse(str, defaultValue = null) {
  ...
  try {
    ...
    console.log(
      `⚠️  Invalid JSON data: "${str.substring(0, 50)}..." - using default value`
    );
    return defaultValue;
  }
}
