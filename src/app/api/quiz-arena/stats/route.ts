...
export async function GET(request: NextRequest) {
  try {
    ...
    const diamondsEarned = user.diamondTransactions.reduce(
    ...
    const recentAccuracy =
      recentQuizzes.length > 0
        ? (recentQuizzes.reduce(
            (sum, quiz) => sum + quiz.correctAnswers / quiz.totalQuestions,
            0
          ) /
    ...
    Object.keys(performanceByDifficulty).forEach((key) => {
      const diff =
        performanceByDifficulty[key as keyof typeof performanceByDifficulty];
      diff.accuracy =
    ...
    const weeklyQuizzes = completedQuizzes.filter(
      (quiz) => quiz.completedAt && quiz.completedAt >= oneWeekAgo
    );

    const weeklyStats = {
      quizzesCompleted: weeklyQuizzes.length,
      questionsAnswered: weeklyQuizzes.reduce(
        (sum, quiz) => sum + quiz.totalQuestions,
        0
      ),
      correctAnswers: weeklyQuizzes.reduce(
        (sum, quiz) => sum + quiz.correctAnswers,
        0
      ),
      accuracy: 0,
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const dayQuestions = dayQuizzes.reduce(
        (sum, quiz) => sum + quiz.totalQuestions,
        0
      );
      const dayCorrect = dayQuizzes.reduce(
        (sum, quiz) => sum + quiz.correctAnswers,
        0
      );
