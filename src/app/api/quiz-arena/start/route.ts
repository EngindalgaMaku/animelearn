...
export async function POST(request: NextRequest) {
  try {
    ...
    if (dbQuestions.length === 0) {
      return NextResponse.json({ error: 'No questions found for the selected category' }, { status: 404 });
    }
    ...
    let allQuestions = dbQuestions.map((q: any, index: number) => {
      ...
      return {
        ...
        difficulty: q.difficulty === 'beginner' ? 1 : q.difficulty === 'intermediate' ? 2 : q.difficulty === 'advanced' ? 3 : 4,
        category: q.category,
        tags: [],
    ...
    if (difficulty === 'progressive') {
      ...
      Object.values(questionsByDifficulty).forEach(difficultyGroup => {
        for (let i = difficultyGroup.length - 1; i > 0; i--) {
          const j = Math.floor(Math.random() * (i + 1));
          [difficultyGroup[i], difficultyGroup[j]] = [difficultyGroup[j], difficultyGroup[i]];
        }
      });
      ...
      selectedQuestions = [];
      const maxLength = Math.max(...Object.values(questionsByDifficulty).map(arr => arr.length));
      
      // Interleave questions from different difficulties for balanced experience
      for (let i = 0; i < maxLength; i++) {
        [1, 2, 3, 4].forEach(diff => {
          if (questionsByDifficulty[diff as keyof typeof questionsByDifficulty][i]) {
            selectedQuestions.push(questionsByDifficulty[diff as keyof typeof questionsByDifficulty][i]);
          }
        });
      }
    ...
    const quiz = await prisma.quiz.create({
      data: {
        ...
        questions: JSON.stringify(selectedQuestions.map((q: any, index: number) => ({ ...q, id: `q_${index}` }))),
        timeLimit: 30,
        difficulty: difficulty === 'mixed' ? 2 : (difficulty === 'easy' ? 1 : difficulty === 'hard' ? 3 : 2),
        diamondReward: 5,
        experienceReward: 10,
        isActive: true
      }
    ...
    const session_data = {
      id: sessionId,
      currentStreak: 0,
      questions: selectedQuestions.map((q: any, index: number) => ({ ...q, id: `q_${index}` })),
