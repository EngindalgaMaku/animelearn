...
export async function GET(req: NextRequest) {
  try {
    ...
    const transformedCodeArenas = codeArenas.map((codeArena: any) => {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Create new code arena activity
    const codeArena = await prisma.codeArena.create({
      data: {
        title,
        slug,
        description,
        ...
        duration: parseInt(estimatedMinutes) || 30,
        tags,
    ...
    return NextResponse.json({
      ...
      id: codeArena.id,
      title: codeArena.title,
      message: "Code Arena activity created successfully"
    });

  } catch (error) {
    console.error("Error creating code arena:", error);
    return NextResponse.json(
      { error: "Failed to create code arena activity" },
      { status: 500 }
    );
  }
}
